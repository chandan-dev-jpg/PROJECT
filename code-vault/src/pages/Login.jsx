...
import { Mail, Lock, Lock as VaultLock, Eye, EyeOff } from 'lucide-react'
...
export default function Login() {
  const [email, setEmail] = useState('chandan@codevault.dev')
  ...
  const handleSubmit = e => {
    ...
    setTimeout(() => {
      login({ email })
      setLoading(false)
      toast.success('Welcome back to Code Vault!')
      navigate('/dashboard')
    }, 700)
  }
    ...
    <div className="grid min-h-screen lg:grid-cols-2">
      <div className="flex items-center justify-center px-6 py-12 sm:px-10">
        ...
        >
          ...
          <p className="mt-1.5 text-sm text-ink-500 dark:text-ink-400">Log in to continue your streak and pick up where you left off.</p>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <Link to="#" className="text-xs font-medium text-brand-600 hover:text-brand-700">Forgot password?</Link>
              </div>
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPassword(s => !s)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-ink-400 hover:text-ink-600">
          ...
          <button className="btn-secondary w-full" onClick={() => toast('Google sign-in is a demo action here.')}>
